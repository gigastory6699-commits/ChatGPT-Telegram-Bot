...
class KeyExhaustedError(Exception):
    """Custom exception for key exhaustion or invalid key."""
    pass
        ...
        while retry_times < self.retry_count:
            ...
            try:
                ...
                if fallback_key and current_key != fallback_key:
                    ...
                    config.Users.set_config(convo_id, "api_key", fallback_key)
                    config.Users.set_config(convo_id, "api_url", fallback_url)
                    config.Users.set_config(convo_id, "engine", fallback_model)

                    config.Users.set_config("global", "api_key", fallback_key)
                    config.Users.set_config("global", "api_url", fallback_url)
                    config.Users.set_config("global", "engine", fallback_model)

                    kwargs['api_key'] = fallback_key
                    kwargs['api_url'] = fallback_url
                    model = fallback_model
                    self.api_key = fallback_key
                    self.api_url = config.BaseAPI(fallback_url)
                    self.engine = fallback_model

                    config.InitEngine(convo_id)

                    # Re-generate the post body with new credentials
                    url, headers, json_post, engine_type = await self.get_post_body(prompt, role, convo_id, model, pass_history, stream=stream, **kwargs)

                    # Reset retry count for the new key
                    retry_times = 0
                    continue
                else:
                    self.logger.error(f"Fallback key also failed or GROQ_API_KEY not set: {e}")
                    raise
