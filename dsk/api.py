...
class DeepSeekAPI:
    ...
    def __init__(self, auth_token: str):
        ...
        try:
            curl_cffi_version = importlib.metadata.version('curl-cffi')
    ...
    def _parse_chunk(self, chunk: bytes) -> Optional[Dict[str, Any]]:
        ...
        try:
            if chunk.startswith(b'data: '):
                ...
                # 1. Handle direct DeepSeek web API format
                if 'v' in data:
                    p = data.get('p')
                    v = data.get('v', '')
                    if not p or p == 'response/content' or p == 'response/text':
                ...
                # 2. Handle standard OpenAI compatibility format
